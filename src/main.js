...
let savedStyle;try{savedStyle=localStorage.getItem('td-map-style-pixel-v1');}catch{}
retro.setStyle(savedStyle||'16');
function setArtStyle(style){retro.setStyle(style);try{localStorage.setItem('td-map-style-pixel-v1',retro.style);}catch{}
  for(const value of ['8','16']){$(`style-${value}`).classList.toggle('active',retro.style===value);$(`style-${value}`).setAttribute('aria-pressed',String(retro.style===value));}
}
$('style-8').onclick=()=>setArtStyle('8');$('style-16').onclick=()=>setArtStyle('16');setArtStyle(retro.style);
...
const rangeRing=new THREE.Mesh(new THREE.RingGeometry(51,52,64),new THREE.MeshBasicMaterial({color:'#87d7b4',transparent:true,opacity:.25,side:THREE.DoubleSide}));rangeRing.rotation.x=-Math.PI/2;rangeRing.visible=false;scene.add(rangeRing);
...
function recalculate(){
  ...
  for(const [i,p] of [pathPoints[0],pathPoints.at(-1)].entries()){
    const ring=new THREE.Mesh(new THREE.RingGeometry(5,6.5,32),new THREE.MeshBasicMaterial({color:i?'#8fe0bd':'#ffc857',side:THREE.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.set(p[0],.9,p[1]);routeGroup.add(ring);
  }
...
function selectBuilding(b){
  ...
  for(const mesh of buildingMeshes){mesh.material.forEach((m,i)=>{m.color.copy(mesh.userData.colors[i]);if(mesh.userData.building.id===b.id)m.color.lerp(new THREE.Color(i?'#78c2a6':'#a9e0ba'),i?.2:.45);});}
...
try{
  ...
  if(new URLSearchParams(location.search).has('test')){
    ...
    window.td.sampleFrameForTesting=()=>{
      ...
      const colors=new Set();for(let i=0;i<bytes.length;i+=4)colors.add((bytes[i]<<16)|(bytes[i+1]<<8)|bytes[i+2]);
      return {colors:[...colors].slice(0,256),colorCount:colors.size,...retro.state};
    };
  }
