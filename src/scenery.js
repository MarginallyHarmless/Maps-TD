...
export function detailedGround(scene,data,renderer){
  ...
  ctx.imageSmoothingEnabled=false;ctx.fillStyle='#88a56b';ctx.fillRect(0,0,canvas.width,canvas.height);
  ...
  for(const a of data.areas){polygon(a.rings);ctx.fillStyle=a.kind==='water'?'#5595a7':a.kind==='playground'?'#c9b58e':a.landcover==='garden'?'#88b45e':'#62954f';ctx.fill('evenodd');}
  ...
  for(let i=0;i<240;i++){
    ...
    ctx.fillStyle='#62954f';ctx.fillRect(x,z,2,1);ctx.fillRect(x+1,z-1,1,1);
  }
  ...
  for(const r of vehicles)stroke(r,r.width+(r.sidewalk==='no'?1:5),'#c7bca3');
  for(const r of vehicles)stroke(r,r.width+.7,'#e4d8bc');
  for(const r of vehicles)stroke(r,r.width,['gravel','unpaved','dirt'].includes(r.surface)?'#a1977e':'#566575');
  for(const r of foot){stroke(r,r.width+.65,'#ded5bb');stroke(r,r.width,r.kind==='cycleway'?'#889584':['gravel','dirt','ground'].includes(r.surface)?'#b7a47d':'#ccc5ad');}
  for(const r of vehicles){
    if(r.width<7)continue;
    const count=Math.max(2,Math.min(5,parseInt(r.lanes)||Math.round(r.width/3.5)));
    ctx.setLineDash([5,7]);
    for(let lane=1;lane<count;lane++)stroke(r,.55,'#e9dfc599',(lane/count-.5)*r.width);
    ctx.setLineDash([]);stroke(r,.13,'#e9dfc56b',r.width/2-.5);stroke(r,.13,'#e9dfc56b',-r.width/2+.5);
  }
  // Treads follow actual mapped stair paths.
  for(const r of foot.filter(r=>r.kind==='steps')){ctx.setLineDash([.25,.7]);stroke(r,r.width,'#8c8e80');ctx.setLineDash([]);}
  // Paving joints on pedestrian surfaces stay clipped to the path geometry.
  for(const r of foot.filter(r=>['paving_stones','sett','cobblestone'].includes(r.surface))){ctx.setLineDash([.07,1.4]);stroke(r,r.width,'#8e8b7933');ctx.setLineDash([]);}
  ...
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(side,side),pixelMaterial({map:tex,roughness:1}));ground.rotation.x=-Math.PI/2;ground.receiveShadow=true;scene.add(ground);
  const base=new THREE.Mesh(new THREE.BoxGeometry(side,9,side),pixelMaterial({color:'#475c4a',roughness:1}));base.position.y=-4.6;scene.add(base);
  ...
  const contact=new THREE.Mesh(new THREE.PlaneGeometry(side,side),new THREE.MeshBasicMaterial({map:shadowTex,transparent:true,opacity:.22,depthWrite:false}));contact.rotation.x=-Math.PI/2;contact.position.y=.025;scene.add(contact);
}
...
export function detailedBuildings(scene,data,buildingMeshes){
  ...
  for(const [i,b] of data.buildings.entries()){
    ...
    const wallMat=pixelMaterial({color:wallColor,roughness:glass ? .32 : .95,metalness:glass ? .2 : 0});
    const topMat=pixelMaterial({color:topColor,roughness:.9,side:THREE.DoubleSide,polygonOffset:true,polygonOffsetFactor:1,polygonOffsetUnits:1});
  ...
  instances(scene,new THREE.PlaneGeometry(1,1),new THREE.MeshBasicMaterial({map:windowTexture(),side:THREE.DoubleSide,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2}),windows,false);
  instances(scene,new THREE.PlaneGeometry(1,1),new THREE.MeshBasicMaterial({side:THREE.DoubleSide,polygonOffset:true,polygonOffsetFactor:-2,polygonOffsetUnits:-2}),glassPanels,false);
  instances(scene,cube,pixelMaterial({roughness:.85}),bands,false);
  // Tiny sill shadows read as speckles at map scale; the building itself casts the shadow.
  instances(scene,cube,pixelMaterial({roughness:.9}),ledges,false);
  instances(scene,cube,pixelMaterial({roughness:.85}),equipment);
  instances(scene,cube,pixelMaterial({roughness:1}),equipmentBases,false);
...
export function detailedTrees(scene,data,mappedPositions){
  ...
  const leaves=instances(scene,geometry,treeMaterial(),items,false);
  const disc=new THREE.CircleGeometry(1,8);disc.rotateX(-Math.PI/2);
  const shade=instances(scene,disc,new THREE.MeshBasicMaterial({color:'#263b33',transparent:true,opacity:.38,depthWrite:false}),shadows,false);
