...
The map uses a pixel-art strategy-game aesthetic. **NOIR** is the new default: a graphite city with pale roofs, midtone walls, dark recessed windows, and clearly separated trees and streets, with amber routes, teal strongholds and defenses, and coral enemies. The interface follows the same dark treatment. **16-BIT** retains the warm 48-color palette, 2× logical pixels, three-step lighting, and original sprite foliage. **8-BIT** uses 24 colors and 3× pixels. All three modes keep crisp edges without final blur or a smooth-color blend. The Noir and 16-bit views draw at twice the final grid resolution in each direction, with up to four MSAA samples, then averages 2×2 samples in linear light before assigning palette colors; 8-bit retains unsmoothed edges. The graphics switch preserves the current game and remembers your preference locally. These are art styles, not emulations of particular consoles. UI text stays at native resolution.
...
![Charles de Gaulle in Noir](artifacts/charles-de-gaulle-noir.png)

![Alba Iulia in Noir](artifacts/alba-iulia-noir-stronghold.png)
...
Noir uses 22 neutral shades plus 17 accent shades. `src/art-direction.js` assigns material-specific grayscale ranges in linear light and applies semantic accents to game objects before the final palette pass. Roofs and ledges sit above midtone facades, windows stay dark, and foliage stands out against the ground. A separate neutral ground texture distinguishes asphalt, curbs, sidewalks, and planted areas; the original colored texture remains available in 8/16-bit styles. Original facade tints are normalized in Noir so dark source colors cannot erase building silhouettes. These changes preserve lighting, shadows, and the existing edge stabilization. Color is never inferred from scenery hues, so roofs, parked cars, and trees stay neutral. The selected roof and pennant, active approach, defenses, enemies, and shots carry the highlights. Switching styles preserves the current wave, placement, and selection.
